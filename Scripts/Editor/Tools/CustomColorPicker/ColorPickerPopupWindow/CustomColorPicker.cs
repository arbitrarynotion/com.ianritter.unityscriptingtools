...
namespace Packages.com.ianritter.unityscriptingtools.Scripts.Editor.Tools.CustomColorPicker.ColorPickerPopupWindow
{
    /// <summary>
    ///     <para>Creates a popup window that lists all of the colors in PresetColors.cs as buttons.</para>
    ///     <para>
    ///         Can be used with either a UnityEngine.Color as a SerializedProperty or a CustomColor directly. Which one it's using
    ///         is determined by the SetTarget method.
    ///     </para>
    /// </summary>
    public class CustomColorPicker : PopupWindowContent
    {
        ...
        {
            if( Math.Abs( _windowSize.x - windowSize.x ) <= 0.01f &&
                Math.Abs( _windowSize.y - windowSize.y ) <= 0.01f ) return;
